...
export function initAdminTab() {
  ...
  subtabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      const targetSec = document.getElementById(`admin-section-${target}`);
      if (targetSec) targetSec.classList.add('active');

      if (target === 'users') loadAdminAccess();
      else if (target === 'settings') loadAdminSettings();
      else if (target === 'keys') loadTorboxKeys();
      else if (target === 'history') loadAdminHistory();
      else if (target === 'announcements') loadAdminAnnouncements();
    });
  });
...
/* ─── Subpanel 1: Users & Access Control ─── */
function initUsersSection() {
  ...
  const btnAdd = document.getElementById('btn-admin-add-access');
  const userIdInput = document.getElementById('admin-access-user-id') as HTMLInputElement | null;
  const listTypeSelect = document.getElementById('admin-access-type') as HTMLSelectElement | null;

  toggleWhitelist?.addEventListener('change', async () => {
    const res = await toggleAdminAccess('whitelist', toggleWhitelist.checked);
    if (res.success) toastSuccess(`Whitelist mode ${toggleWhitelist.checked ? 'enabled' : 'disabled'}`);
    else toastError(res.error || 'Failed to toggle whitelist');
  });

  toggleBlacklist?.addEventListener('change', async () => {
    const res = await toggleAdminAccess('blacklist', toggleBlacklist.checked);
    if (res.success) toastSuccess(`Blacklist mode ${toggleBlacklist.checked ? 'enabled' : 'disabled'}`);
    else toastError(res.error || 'Failed to toggle blacklist');
  });

  btnAdd?.addEventListener('click', async () => {
    const userId = userIdInput?.value.trim() || '';
    const listType = (listTypeSelect?.value || 'whitelist') as 'whitelist' | 'blacklist';
    ...
    const res = await addAdminAccess(userId, listType);
    if (res.success) {
      toastSuccess(`User added to ${listType}`);
  ...
  document.getElementById('admin-access-list-container')?.addEventListener('click', async (e) => {
    ...
    if (res.success) {
      toastSuccess('User removed from list');
      loadAdminAccess();
    } else {
      ...
      <div class="history-item-card">
        <div class="history-item-top" style="align-items: center; gap: 14px;">
          ...
          <div style="min-width: 0; flex: 1;">
            <div class="history-item-title mono" style="font-size: 14px; font-weight: 600; margin-bottom: 3px;">
          ...
          <button class="btn btn-secondary btn-icon btn-sm" data-remove-user="${escapeHtml(entry.user_id)}" title="Remove from list">
...
async function loadAdminSettings() {
  ...
  if (publicApiEl) publicApiEl.checked = map.public_api_enabled !== false && String(map.public_api_enabled) !== 'false';
...
function initKeysSection() {
  ...
  const keyInput = document.getElementById('admin-new-torbox-key') as HTMLInputElement | null;
  ...
  btnAdd?.addEventListener('click', async () => {
    ...
    if (res.success) {
      toastSuccess('TorBox API Key added to pool');
  ...
  document.getElementById('admin-keys-container')?.addEventListener('click', async (e) => {
    ...
    const indexStr = target.getAttribute('data-delete-index');
...
async function loadTorboxKeys() {
  ...
  container.innerHTML = keys
    .map((k, index) => {
      ...
      const errorInfo = !isValid && k.error ? `<span style="color: var(--status-danger); font-size: 11px;">(${escapeHtml(k.error)})</span>` : '';
      ...
      <div class="history-item-card">
        <div class="history-item-top">
          ...
          <button class="btn btn-secondary btn-icon btn-sm" data-delete-index="${index}" title="Delete Key">
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
          </button>
        </div>
      </div>
  ...
  container.innerHTML = filtered
    ...
    <div class="history-item-card">
      <div class="history-item-top">
        ...
        <a href="${item.browse_url}" class="btn btn-secondary btn-icon btn-sm" title="Browse">
  ...
  container.innerHTML = list
    ...
    <div class="history-item-card">
      <div class="history-item-top">
        ...
        <button class="btn btn-secondary btn-icon btn-sm" data-delete-announcement="${escapeHtml(ann.id)}" title="Delete Announcement">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f87171" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
        </button>
      </div>
    </div>
